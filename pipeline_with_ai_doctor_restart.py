#!/usr/bin/env python3
"""
Pipeline Wrapper with Sourcery Restart Support

This wrapper script handles the complete Sourcery + Pipeline integration:
1. Runs the master automation pipeline
2. If Sourcery applies changes (exit code 42), restarts the pipeline  
3. Continues until no more Sourcery changes are needed
4. Prevents infinite restart loops

Usage:
    python pipeline_with_sourcery_restart.py [pipeline args...]
"""
...
def run_pipeline_with_restart(args):
    """Run the pipeline with automatic restart support for Sourcery changes"""
    ...
    print("=" * 80)
    print("PIPELINE WITH SOURCERY RESTART SUPPORT")
    print("=" * 80)
    ...
    while restart_count < max_restarts:
        if restart_count > 0:
            print(f"\n🔄 RESTART #{restart_count} - Sourcery applied changes")
            print("=" * 60)
        ...
        try:
            ...
            if exit_code == 42:
                # Sourcery applied changes - restart needed
                restart_count += 1
                print(f"🔄 Sourcery restart requested (attempt {restart_count}/{max_restarts})")
                
                # Check for restart marker file
                restart_marker = Path.cwd() / '.sourcery_restart_pending'
                ...
                if restart_count < max_restarts:
                    print("Restarting pipeline with fresh Sourcery changes...")
                    continue
                else:
                    print("⚠️  Maximum restarts reached - stopping to prevent infinite loop")
                    print("This usually means Sourcery keeps finding new issues to fix.")
                    print("Consider reviewing the changes manually or adjusting Sourcery config.")
                    return 1
