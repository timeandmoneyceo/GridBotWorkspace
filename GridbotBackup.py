...
class WebSocketManager:
    ...
    def on_coinbase_message(self, ws, message):
        try:
            ...
            if data.get("type") == "match":
                ...
                if any(key not in data for key in ["price", "time"]):
    ...
    def init_ws_candle_buffer(self):
        ...
        try:
            ...
            if ohlcv := self.exchange.fetch_ohlcv(
                config.SYMBOL,
                timeframe,
                since=since_init_ms,
                limit=fetch_limit_init,
            ):
    ...
    def aggregate_ws_candle(self, ticker_data):
        ...
        try:
            if any(key not in ticker_data for key in ["price", "time"]):
            ...
            if self.last_candle_time is None or candle_time > self.last_candle_time:
                if self.current_candle is not None:
                    ...
                    if temp_buffer.empty or temp_buffer.isna().all().all():
            ...
            elif self.current_candle is not None: # Ensure current_candle exists
        ...
        try:
            ...
            if len(recent_candles) < needed_candles:
                ...
                try:
                    ...
                    if ohlcv := self.exchange.fetch_ohlcv(
                        config.SYMBOL, timeframe, since=since_ms, limit=fetch_limit
                    ):
                        historical_df = pd.DataFrame(
                            ohlcv,
                            columns=["timestamp", "open", "high", "low", "close", "volume"]
...
def can_place_feature_order_with_global_spacing(bot_state, price, min_spacing=0.005):
    ...
    return last_exec is None or abs(last_exec - price) >= min_spacing * price
...
def online_update_sklearn(model, scaler, X, y, lookback=80):
    ...
    try:
        ...
        if len(X) < lookback:
            ...
            return model
        if any(f not in X.columns for f in features):
            logger.error(f"Missing features in X: {[f for f in features if f not in X.columns]}")
            return model

        X_seq = [
            X[features].iloc[i : i + lookback].values.flatten()
...
def online_update_pytorch(model, feature_scaler, target_scaler, X, y, device='cpu'):
    try:
        ...
        if missing_features := [f for f in features if f not in X.columns]:
...
def retrain_ml_models(data, scaler_sklearn_rf, scaler_pytorch, scaler_xgb, scaler_meta):
    try:
        ...
        criterion = nn.MSELoss()
        for _ in range(config.PYTORCH_NUM_EPOCHS):
            pytorch_model.train()
            optimizer.zero_grad()
...
def _update_live_data_from_websocket(websocket_manager, main_ohlcv_df, temp_raw_trades_df):
    ...
    try:
        if not new_ohlcv_candles.empty and len(new_ohlcv_candles) >= config.BOLLINGER_WINDOW:
            ...
            dynamic_defaults = {
                "rsi": 50.0 if new_ohlcv_candles["rsi"].isna().all() else new_ohlcv_candles["rsi"].mean(),
...
def compute_volatility(ohlcv_df, periods=14, timeframe_minutes=1):
    ...
    if any(col not in ohlcv_df.columns for col in required_columns):
    ...
    if ensemble_vol.isna().any():
        mean_vol = 0.035 if ensemble_vol.isna().all() else ensemble_vol.mean()
...
def train_meta_model(prediction_history, min_samples=1):
    ...
    try:
        ...
        target = "actual_price"
        if any(
            col not in prediction_history.columns
        ...
        if meta_model is None or meta_scaler is None or not hasattr(meta_scaler, "scale_"):
            ...
            if history_rows >= 1:  # Retry training if sufficient history
                try:
                    ...
                    if meta_model is not None and meta_scaler is not None and hasattr(meta_scaler, "scale_"):
                        ...
                        logger.error("Meta-model training failed; attempting to load saved files")
            ...
            try:
                if os.path.exists("client_meta_model.pkl") and os.path.exists("client_meta_scaler.pkl"):
                    ...
                    logger.info(
                        "Loaded saved meta-model and scaler from client_meta_model.pkl and client_meta_scaler.pkl"
                    )
...
def predict_pytorch_price(model, feature_scaler, target_scaler, lookback, recent_data, current_price, last_valid_prediction=None):
    try:
        ...
        if missing_features := [f for f in features if f not in data.columns]:
        ...
        for attempt in range(3):
            ...
            if usd >= cost:
                try:
                    ...
                    if prior_buy_price := buy_prices.pop(order_id, None):
...
def adjust_grid_with_funds_check(ws_manager, exchange):
    try:
        ...
        def count_open_feature_orders_for_side(bot_state, feature, side):
            ...
            return sum(bool(o.get("feature", "base").lower() == feature.lower() and o.get("status", "open") == "open")
                   for o in orders)
        def count_total_open_feature_orders_for_side(bot_state, side):
            orders = bot_state["buy_orders"] if side == "buy" else bot_state["sell_orders"]
            return sum(bool(o.get("feature", "base").lower() in features_list and o.get("status", "open") == "open")
                   for o in orders)
...
def place_order(side, price, size, ws_manager, exchange):
    try:
        ...
        if is_feature and not can_place_feature_order_with_global_spacing(bot_state, price, min_feature_spacing):
...
def order_to_dict(order, status=None):
    return {
        ...
        "status": (
            status if status is not None else order.get("status", "unknown")
        ),
...
def count_open_feature_orders(bot_state, feature, side):
    ...
    """
    orders = bot_state["buy_orders"] if side == "buy" else bot_state["sell_orders"]
    return sum(bool(o.get("feature") == feature and o.get("status", "open") == "open")
           for o in orders)
...
def count_total_open_feature_orders(bot_state):
    ...
    """
    return sum(bool(o.get("feature") not in (None, "base") and o.get("status", "open") == "open")
           for o in bot_state["buy_orders"] + bot_state["sell_orders"])

def count_total_open_base_orders(bot_state):
    """
    Count all open base grid orders (across both sides).
    """
    return sum(bool(o.get("feature", "base") == "base" and o.get("status", "open") == "open")
           for o in bot_state["buy_orders"] + bot_state["sell_orders"])
...
def recommend_dynamic_params(usd_balance, eth_balance, volatility_val, meta_confidence, recent_accuracy=None, price_val=None):
    ...
    min_spacing = 0.005
    total_value = usd_balance + eth_balance * (price_val or 1)
    eth_pct = (eth_balance * (price_val or 1)) / total_value if total_value > 0 else 0.5
...
def cancel_and_replace_order(order, new_price, side, bot_state):
    ...
    try:
        ...
        if 'exchange' in globals() and symbol:
            ...
            logger.warning(
                "[ORDER SPACING] Could not place new order: exchange or symbol not available."
            )
    except Exception as e:
...
def run_bot():
    ...
    valid_models = sum(bool(model is not None)
                   for model in [sklearn_rf_model, sklearn_sgd_model, pytorch_model, xgb_model, meta_model])
    if valid_models < 2:
    ...
    try:
        ...
        if updated and not ohlcv_df.empty:
            ...
            logger.debug(
                f"ohlcv_df after timestamp fix: first_timestamp={'N/A' if ohlcv_df.empty else ohlcv_df['timestamp'].iloc[0]}, "
                f"last_timestamp={'N/A' if ohlcv_df.empty else ohlcv_df['timestamp'].iloc[-1]}, "
                f"last_close={'N/A' if ohlcv_df.empty else ohlcv_df['close'].iloc[-1]}"
            )
    ...
    if (
        ...
        if soft_feature_order_cap is not None:
            feature_count = sum(bool(o.get("feature") == "rsi" and o.get("status", "open") == "open")
                            for o in bot_state["buy_orders"])
    ...
    if (
        ...
        if soft_feature_order_cap is not None:
            feature_count = sum(bool(o.get("feature") == "bollinger" and o.get("status", "open") == "open")
                            for o in bot_state["buy_orders"])
            if feature_count >= soft_feature_order_cap:
    ...
    if (
        ...
        if soft_feature_order_cap is not None:
            feature_count = sum(bool(o.get("feature") == "macd" and o.get("status", "open") == "open")
                            for o in bot_state["buy_orders"])
            if feature_count >= soft_feature_order_cap:
    ...
    try:
        ...
        while not shutdown_event.is_set():
            try:
                ...
                current_time = time.time()
                current_volume = 0.0 if ohlcv_df.empty else ohlcv_df["volume"].iloc[-1]
                ...
                if len(recent_trade_data) >= config.PYTORCH_LOOKBACK:
                    try:
                        ...
                        log_ref_timestamp = pd.Timestamp.now(tz="UTC") if recent_trade_data.empty else recent_trade_data['timestamp'].iloc[-1]
                        log_ref_current_close = current_price if recent_trade_data.empty else recent_trade_data['close'].iloc[-1]
                        ...
                        if bot_state.get("meta_model") and bot_state.get("meta_scaler") and all(np.isfinite([sklearn_predicted, pytorch_predicted, xgb_predicted])):
                            volatility = 0.1 if recent_trade_data.empty else recent_trade_data["volatility"].iloc[-1]
                            try:
                ...
                try:
                    logger.info("Checking for data refresh: ohlcv_df_rows=%d, last_timestamp=%s",
                                len(ohlcv_df), 'None' if ohlcv_df.empty else ohlcv_df['timestamp'].iloc[-1])
                        ...
                        try:
                            ...
                            try:
                                ...
                                if updated and not ohlcv_df.empty:
                                    ...
                                    close_mean = current_price if ohlcv_df.empty else ohlcv_df["close"].mean()
                                    defaults = {
                                        ...
                                        "vwap": close_mean,
                                        "predicted_price": current_price if ohlcv_df.empty else ohlcv_df["close"].iloc[-1],
                                        "grid_level": 0,
                ...
                try:
                    ...
                    for order_list, is_buy in [
                        ...
                        base_order_count = sum(bool(o.get("feature", "base") == "base")
                                           for o in order_list)
                        feature_order_counts = {f: sum(bool(isinstance(o, dict) and o.get("feature") == f)
                                                   for o in order_list) for f in expanded_feature_list}
                                ...
                                if can_place_feature_order('rsi', 'buy', price_val):
                                    ...
                                    while retry_count < max_retries:
                                        try:
                                            ...
                                            best_bid = float(orderbook['bids'][0][0]) if orderbook['bids'] else None
                                            best_ask = float(orderbook['asks'][0][0]) if orderbook['asks'] else None
                                            logger.info(f"[RSI] Order book: best_bid={best_bid or 'N/A'}, best_ask={best_ask or 'N/A'}")
                                            ...
                                            if best_ask is not None and price < best_ask:
                                                ...
                                                logger.warning(f"[RSI] Buy price {price:.2f} not below best ask {best_ask or 'N/A'}, retrying...")
                                                time.sleep(5)
                                            retry_count += 1
                                        except Exception as e:
                                            if "INVALID_LIMIT_PRICE_POST_ONLY" in str(e):
                                    ...
                                    while retry_count < max_retries:
                                        try:
                                            ...
                                            if best_bid is not None and price >= best_bid + min_tick:
                                                ...
                                                logger.warning(f"[RSI] Sell price {price:.2f} not above best bid {best_bid or 'N/A'} + min_tick, retrying...")
                                                time.sleep(5)
                                ...
                                if can_place_feature_order('bollinger', 'buy', price_val):
                                    ...
                                    while retry_count < max_retries:
                                        try:
                                            ...
                                            best_bid = float(orderbook['bids'][0][0]) if orderbook['bids'] else None
                                            best_ask = float(orderbook['asks'][0][0]) if orderbook['asks'] else None
                                            logger.info(f"[BOLLINGER] Order book: best_bid={best_bid or 'N/A'}, best_ask={best_ask or 'N/A'}")
                                            if best_bid is not None and best_ask is not None:
                                                mid_price = (best_bid + best_ask) / 2
                                                new_price = min(mid_price - min_tick, best_ask - min_tick)
                                                price = float(exchange.price_to_precision(config.SYMBOL, new_price))
                                            if best_ask is not None and price < best_ask:
                                                ...
                                                logger.warning(f"[BOLLINGER] Buy price {price:.2f} not below best ask {best_ask or 'N/A'}, retrying...")
                                                time.sleep(1)
                                                if retry_count >= 2 and best_bid is not None and best_ask is not None:
                                                    mid_price = (best_bid + best_ask) / 2
                                                    new_price = min(mid_price - min_tick, best_ask - min_tick)
                                    ...
                                    while retry_count < max_retries:
                                        try:
                                            ...
                                            if best_bid is not None and price >= best_bid + min_tick:
                                                ...
                                                logger.warning(f"[BOLLINGER] Sell price {price:.2f} not above best bid {best_bid or 'N/A'} + min_tick, retrying...")
                                                time.sleep(1)
                                                if retry_count >= 2 and best_bid is not None and best_ask is not None:
                                                    mid_price = (best_bid + best_ask) / 2
                                                    new_price = max(mid_price + min_tick, best_bid + min_tick)
                                ...
                                if can_place_feature_order('macd', 'buy', price_val):
                                    ...
                                    while retry_count < max_retries:
                                        try:
                                            ...
                                            best_bid = float(orderbook['bids'][0][0]) if orderbook['bids'] else None
                                            best_ask = float(orderbook['asks'][0][0]) if orderbook['asks'] else None
                                            logger.info(f"[MACD] Order book: best_bid={best_bid or 'N/A'}, best_ask={best_ask or 'N/A'}")
                                            if best_bid is not None and best_ask is not None:
                                                mid_price = (best_bid + best_ask) / 2
                                                new_price = min(mid_price - min_tick, best_ask - min_tick)
                                                price = float(exchange.price_to_precision(config.SYMBOL, new_price))
                                            if best_ask is not None and price < best_ask:
                                                ...
                                                logger.warning(f"[MACD] Buy price {price:.2f} not below best ask {best_ask or 'N/A'}, retrying...")
                                                time.sleep(1)
                                                if retry_count >= 2 and best_bid is not None and best_ask is not None:
                                                    mid_price = (best_bid + best_ask) / 2
                                                    new_price = min(mid_price - min_tick, best_ask - min_tick)
                                    ...
                                    while retry_count < max_retries:
                                        try:
                                            ...
                                            if best_bid is not None and price >= best_bid + min_tick:
                                                ...
                                                logger.warning(f"[MACD] Sell price {price:.2f} not above best bid {best_bid or 'N/A'} + min_tick, retrying...")
                                                time.sleep(1)
                                                if retry_count >= 2 and best_bid is not None and best_ask is not None:
                                                    mid_price = (best_bid + best_ask) / 2
                                                    new_price = max(mid_price + min_tick, best_bid + min_tick)
                        ...
                        base_open = sum(bool(o.get("feature", "base") == "base" and o.get("status", "open") == "open")
                                    for o in order_list)
                        feature_names = ["rsi", "bollinger", "macd", "ema", "atr", "volatility", "vwap"]
                        feature_open = {f: sum(bool(o.get("feature") == f and o.get("status", "open") == "open")
                                           for o in order_list) for f in feature_names}
                    ...
                    new_entry = pd.DataFrame(
                        {
                            ...
                            "actual_price": [
                                current_price if recent_trade_data.empty else recent_trade_data["close"].iloc[-1]
                            ],
                        }
                    )
                ...
                try:
                    if sklearn_predicted and pytorch_predicted and xgb_predicted and meta_predicted:
                        ...
                        actual_price = current_price if recent_data.empty else recent_data["close"].iloc[-1]
                        ...
                        meta_predictions_list = list(bot_state["recent_meta_outputs"]) or [meta_predicted if meta_predicted is not None and np.isfinite(meta_predicted) else current_price]
                        ...
                        historical_mean = 0.0 if historical_data.empty else historical_data['volume'].mean()
                        volume_factor = (
                            min(
                    ...
                    orders_to_send.extend(
                        [
                            ...
                            },
                            {
                                ...
                                "price": float(current_price if ohlcv_df.empty else ohlcv_df["close"].iloc[-1]),
                                "volume": float(
