...
def translate_message_for_ml(message):
    ...
    if any(feature not in message for feature in required_features):
        ...
        if command:
            if command == "optimize":
                ...
                if client_id in ["Long Term ML", "Wscat"]:  # Allow Long Term ML and Wscat
                    if client_target := self.clients.get("Client"):
...
if __name__ == "__main__":
    try:
        ...
        raise
"""
GridBot WebSocket Server

This server exposes a lightweight WebSocket interface for GridBot components
and an optional /metrics HTTP endpoint for the Autonomous Strategy Manager.

Features:
- Runtime config hot-reload via runtime_config.json (if available)
- WebSocket broadcast and simple command handling (identify/echo/config_update)
- Optional HTTP /metrics endpoint for live telemetry
"""
...
try:
    # Prefer local import when running as a module
    from . import runtime_config  # type: ignore
except Exception:
    # Fallback to top-level import when executed as a script
    try:
...
try:
    ...
    raise SystemExit(
        f"websocket_server package missing or failed to import: {e}"
    ) from e


logger = logging.getLogger("gridbot_ws_server")
...
def _on_config_update(cfg):
    ...
    logger.info(f"Runtime params updated: {RUNTIME_PARAMS}")


def _init_runtime_config_watcher():
    global _rc_thread, _rc_stop
    if runtime_config is None:
        logger.info("runtime_config not available; skipping hot-reload watcher")
        return
    try:
        ...
        logger.info("Started runtime_config watcher")
    except Exception as e:
        logger.warning(f"Failed to start runtime_config watcher: {e}")
        _rc_thread = None
        _rc_stop = None


def start_metrics_http(port: int = 8123):
    """Optional tiny HTTP metrics server to provide live metrics to ASM."""
    try:
        ...
        th.start()
        logger.info(f"Metrics HTTP server started on :{port}")
        return srv
    except Exception as e:
...
def on_message_received(client, server, message):
    ...
    action = data.get("action") or data.get("command") or "echo"

    if action == "identify":
        reply = {
            "action": "identify",
            "status": "success",
            "client": str(client.get("id")),
            "runtime_params": RUNTIME_PARAMS,
        }
        server.send_message(client, json.dumps(reply))
        logger.info(f"Identify handshake completed for client {client.get('id')}")
        return

    if action in {"optimize", "config_update"}:
        # Echo an applied response; the actual application may happen in other components listening
        reply = {
            "action": action,
            "status": "applied",
        ...
        server.send_message(client, json.dumps(reply))
        logger.info(f"Applied command {action} from client {client.get('id')}")
        return

    # default echo
    server.send_message(client, json.dumps({"action": "echo", "received": data}))


def start_ws_server(host: str = "0.0.0.0", port: int = 8765) -> WebsocketServer:
    srv = WebsocketServer(host=host, port=port, loglevel=logging.INFO)
    srv.set_fn_new_client(on_new_client)
    srv.set_fn_client_left(on_client_left)
    srv.set_fn_message_received(on_message_received)
    th = threading.Thread(target=srv.run_forever, daemon=True)
    th.start()
...
def main():
    ...
    metrics_enabled = os.environ.get("GRIDBOT_METRICS_HTTP", "0") in ("1", "true", "True")
    metrics_port = int(os.environ.get("GRIDBOT_METRICS_PORT", "8123"))
    if metrics_enabled:
        start_metrics_http(metrics_port)

    # runtime config watcher
    _init_runtime_config_watcher()

    # WS server
    ws_port = int(os.environ.get("GRIDBOT_WS_PORT", "8765"))
    srv = start_ws_server(port=ws_port)

    # Keep the main thread alive, allow Ctrl+C to stop
